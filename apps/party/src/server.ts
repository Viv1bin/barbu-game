 ...
 */
export default class BarbuServer implements Party.Server {
  ...
  private handleJoin(sender: Party.Connection, msg: Extract<ClientMsg, { t: 'JOIN' }>) {
  ...
  private handleSeat(sender: Party.Connection, msg: Extract<ClientMsg, { t: 'SEAT' }>) {
  ...
  private handleNewGame(sender: Party.Connection) {
  ...
  private async handleAction(sender: Party.Connection, action: Action) {
  ...
  private sendMsg(conn: Party.Connection, msg: ServerMsg) {
    conn.send(JSON.stringify(msg));
  }

  private sendError(conn: Party.Connection, msg: string) {
