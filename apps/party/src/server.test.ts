...
import BarbuServer, { TIMING } from './server.js';
import type { ClientMsg, ServerMsg } from '@barbu/engine';

// Faux harnais PartyKit : capture les messages envoyés à chaque connexion.
class FakeConn {
  sent: ServerMsg[] = [];
...
async function driveToDone(server: BarbuServer) {
...
describe('serveur en ligne', () => {
  it('vue caviardée : un joueur ne voit que sa propre main', async () => {
    ...
    const server = new BarbuServer(room as never);
    const host = new FakeConn('h');
    room.conns.push(host);

    server.onConnect(host as never);
    server.onMessage(msg({ t: 'JOIN', profileId: 'p-host', name: 'Hôte', avatar: '🙂' }), host as never);
    for (const seat of [1, 2, 3] as const) {
      server.onMessage(msg({ t: 'SEAT', seat, kind: 'bot', level: 'facile' }), host as never);
    }
    server.onMessage(msg({ t: 'START' }), host as never);
    await flush();
  ...
  it('coup illégal / hors-tour → ERROR, état inchangé', async () => {
    ...
    const host = new FakeConn('h');
    room.conns.push(host);
    server.onMessage(msg({ t: 'JOIN', profileId: 'p', name: 'Hôte', avatar: '🙂' }), host as never);
    // Action alors qu'aucune partie n'est lancée → ERROR.
    host.sent = [];
    server.onMessage(msg({ t: 'ACTION', action: { t: 'PLAY_CARD', player: 0, card: { suit: 'H', rank: 2 } } }), host as never);
    await flush();
